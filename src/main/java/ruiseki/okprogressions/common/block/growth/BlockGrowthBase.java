...
public class BlockGrowthBase extends ConfigurableBlockContainer {
    ...
    private int getGrowthCrystalTickRate() {
        int tickRate = this.growthLvl == 1 ? GeneralConfig.growthTicks
            : this.growthLvl == 2 ? GeneralConfig.upgradeGrowthTicks
                : this.growthLvl == 3 ? GeneralConfig.upgradeTwoGrowthTicks : 40;
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        if (!GeneralConfig.particle) return;
        if (rand.nextInt(GeneralConfig.particleTicks) != 0) return;
    ...
    public TileEntity createTileEntity(World world, int metadata) {
        TEGrowth teGrowth = (TEGrowth) super.createTileEntity(world, metadata);
        return teGrowth.setStats(growthLvl, range, rangeY);
    }
}
